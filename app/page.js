...
export default function Home() {
  ...
  return (
    <div style={{ 
      // Synchronized Rich Green Gradient Backdrop Escrow Configuration
      ...
      <div style={{ padding: '16px 20px', position: 'sticky', top: 0, zIndex: 1000 }}>
        <nav style={{
          ...
          {/* Logo Brand Anchor Node with Neon Glow Parameters on click */}
          ...
          {/* Desktop Right Navigation Array with Hover Underlines */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '28px' }} className="desktop-nav-menu">
            ...
            {isLoggedIn ? (
              <>
                ...
                {/* Fixed: Settings now only shows if logged in */}
        ...
        {/* Responsive Dropdown Architecture Segment for Mobile Frameworks */}
        {isDropdownOpen && (
          <div style={{
            margin: '8px auto 0',
            maxWidth: '1200px',
          ...
          >
            <Link href="/" onClick={() => setIsDropdownOpen(false)} style={{ textDecoration: 'none', color: '#1D9E75', fontWeight: 700, fontSize: '14px' }}>Home</Link>
            {isLoggedIn ? (
              <>
                ...
                {/* Fixed: Settings now only shows if logged in inside mobile menu too */}
              ...
              <>
                <Link href="/login" onClick={() => setIsDropdownOpen(false)} style={{ textDecoration: 'none', color: '#4b5563', fontWeight: 600, fontSize: '14px' }}>Sign In</Link>
              </>
            )}
          </div>
        )}
      ...
      {/* Hero Section Container with Enhanced Green & White Translucent Gradient Backdrop */}
      <section style={{ 
        maxWidth: '900px', 
        ...
        borderRadius: '24px',
        border: '1px solid rgba(0, 0, 0, 0.08)',
        boxShadow: '0 12px 40px rgba(29, 158, 117, 0.03)'
      }}>
      ...
      {/* How it works Segment with matching Translucent Filters */}
      <section id="how" style={{ maxWidth: '940px', margin: '40px auto', padding: '0 20px' }}>
        <div style={{ 
          ...
          <h2 style={{ textAlign: 'center', fontSize: '36px', fontWeight: 800, color: '#111827', margin: '0 0 48px', letterSpacing: '-0.02em' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '24px' }}>
            {[
              ...
              >
                ...
                <h4 style={{ margin: '0 0 8px', color: '#111827', fontSize: '15px', fontWeight: 700 }}>{s.title}</h4>
                <p style={{ margin: 0, color: '#374151', fontSize: '13px', lineHeight: 1.55, fontWeight: 500 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '24px' }}>
          {[
            ...
            <div key={s.label} style={{ 
              ...
              <p style={{ margin: '0 0 6px', fontWeight: 700, color: '#111827', fontSize: '15px' }}>{s.label}</p>
              <p style={{ margin: 0, color: '#4b5563', fontSize: '12px', lineHeight: 1.5, fontWeight: 500 }}>{s.sub}</p>
            </div>
          ))}
        </div>
      </section>

      {/* CTA Bottom Section with Deep Flat Contrast Accent */}
      <section style={{ background: '#111827', borderTop: '1px solid rgba(0, 0, 0, 0.15)', padding: '80px 20px', textAlign: 'center' }}>
        <h2 style={{ color: 'white', fontSize: '38px', fontWeight: 800, margin: '0 0 12px', letterSpacing: '-0.02em' }}>Stop chasing invoices.</h2>
        <p style={{ color: '#9ca3af', fontSize: '16px', margin: '0 0 32px', fontWeight: 500 }}>Deploy your first escrow project agreement layout in under 2 minutes.</p>
        <Link href={isLoggedIn ? "/dashboard" : "/login"} style={{ textDecoration: 'none' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '24px', flexWrap: 'wrap' }}>
          {['No hidden setup costs', 'No software subscription', 'Cancel milestones anytime'].map(t => (
            <span key={t} style={{ color: '#9ca3af', fontSize: '13px', fontWeight: 600 }}>✓ {t}</span>
          ))}
        </div>
      </section>
      ...
      <style jsx global>{`
        ...
        /* Responsive Framework Visibility Toggle Constraints */
